...
public class Main {
	...
	public void run(PrintWriter out) {
		...
		for (int n = 0; n < N; n++) {
			for (int i = 0; i < (1 << n); i++) {
				if (cache[i] >= 0 && (i & outs[n]) == i) { // 条件を満たす連結成分であるものに自分を足しても大丈夫か
					cache[i + (1 << n)] = 1;
				}
			}
		}
...
//		debug(cache);
		out.println(recurcive((1 << N) - 1, outs));
		
	}
	
	private long[] cache;
	
	private long recurcive(int set, int[] outs) {
		if (cache[set] >= 0) return cache[set];
		long min = Long.MAX_VALUE;
		for (int sub = ((set - 1) & set); sub > 0; sub = ((sub-1) & set)) {
			long tmp = recurcive(sub, outs) + recurcive(set & (~sub), outs);
			if (tmp < min) min = tmp;
		}
