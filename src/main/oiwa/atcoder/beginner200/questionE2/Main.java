package oiwa.atcoder.beginner200.questionE2;
...
public class Main {
	...
	public void run(PrintWriter out) {
		...
		long index = 0;
		for (; sum <= 3*N; sum++) {
			long s = Math.min(sum,  (3+3*N) - sum);
			long delta = (s - 2) * (s - 1) / 2;
			if (s > N) {
				s -= N;
				delta -= (s - 2) * (s - 1) / 2 * 3;
			}
...
//		debug("sum=" + sum + " " + index);
		...
		long a = Math.max(1, sum - 2*N);
		for (; a <= N; a++) {
			long s = sum - a;
			long delta = Math.min(N, s - 1) - Math.max(1, s - N) + 1;
//			debug(a + " " + delta + " " + (delta+index));
			delta += index;
			if (delta >= K) {
				break;
			}
...
//		debug("a=" + a + " " + index);
		...
		long b = clamp(K - index, sum - a - N, N);
