package oiwa.atcoder.beginner192.questionF2;
...
//		out.println(sc.next());
		...
		for (int k = 1; k <= N; k++) {
			final long[][][] res = new long[N+1][k+1][k]; // n, k, m     X - sum
			
			for (int n = 0; n <= N; n++) {
				for (int kk = 0; kk <= k; kk++) {
					for (int m = 0; m < k; m++) {
						res[n][kk][m] = Long.MAX_VALUE;
					}
				}
			}
			...
			for (int n = 0; n < N; n++) {
				for (int kk = 0; kk <= n && kk <= k; kk++) {
					for (int m = 0; m < k; m++) {
						long next = res[n][kk][m];
						if (next != Long.MAX_VALUE) {
							// n+1番目を追加しない場合
							res[n+1][kk][m] = Math.min(next, res[n+1][kk][m]);
							
							if (kk < k) { // 追加する場合
								next -= as[n];
								res[n+1][kk+1][(int) (next % k)] = Math.min(next, res[n+1][kk+1][(int) (next % k)]);
							}
						}
					}
				}
			}
			if (res[N][k][0] != Long.MAX_VALUE) result = Math.min(result, res[N][k][0] / k);
		}
