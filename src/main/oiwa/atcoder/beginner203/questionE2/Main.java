...
public class Main {
	...
	public void run(PrintWriter out) {
		...
		Pawn[] pawnlist = new Pawn[M];
		for (int i = 0; i < M; i++) {
			int x = sc.nextInt();
			int y = sc.nextInt();
//			Set<Integer> set;
//			if (pawns.containsKey(x)) set = pawns.get(x);
//			else pawns.put(x, set = new HashSet<>());
//			set.add(y);
			pawnlist[i] = new Pawn(x, y);
		}
		
		Arrays.sort(pawnlist, (e1, e2) -> Integer.compare(e1.x, e2.x));
		Map<Integer, Set<Integer>> pawns = new HashMap<>();
		List<Integer> xs = new ArrayList<>();
		{
			...
			for (Pawn p : pawnlist) {
				if (prev != p.x) {
					prev = p.x;
					xs.add(prev);
					pawns.put(p.x, new HashSet<>());
				}
				pawns.get(p.x).add(p.y);
			}
		}
		...
		possible.add(N);
		for (int x : xs) {
			Set<Integer> pawn = pawns.get(x);
			Set<Integer> diffRem = new HashSet<>(pawn);
			Set<Integer> diffAdd = new HashSet<>();
			for (int p : pawn) {
				if (possible.contains(p-1)) diffAdd.add(p);
				else if (possible.contains(p+1)) diffAdd.add(p);
			}
			possible.removeAll(diffRem);
			possible.addAll(diffAdd);
		}
