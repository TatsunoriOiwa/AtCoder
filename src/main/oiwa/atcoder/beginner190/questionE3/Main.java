package oiwa.atcoder.beginner190.questionE3;
...
public class Main {
	...
	public void run(PrintWriter out) {
		...
		final int[] C = sc.nextIntArray(K, -1);
		long[][] dists = new long[K][K];
		for (int ci = 0; ci < K; ci++) {
			long[] tmp = bfs(outs, C[ci], N);
			for (int i = 0; i < K; i++) {
				if ((dists[ci][i] = tmp[C[i]]) == Long.MAX_VALUE) {
					out.println(-1);
					return;
				}
			}
		...
		// 巡回セールスマン
		cache = new long[1 << K][K]; // [ セット ][ 開始地点 ]
		for (int i = 1; i < 1 << K; i++) {
			Arrays.fill(cache[i], -1);
		}
//		for (int i = 0; i < K; i++) {
//			Arrays.fill(cache[1 << i], 0);
//		}
		
		long min = Long.MAX_VALUE;
		for (int i = 0; i < K; i++) {
			long tmp = dist(((1 << K) - 1) & ~ (1 << i), i, dists, K);
			if (tmp < min) min = tmp;
		}
//		for (int i = 0; i < 1 << K; i++) debug(cache[i]);
		out.println(min+1);
	}
	private long[][] cache;
	private long dist(int set, int from, long[][] dists, int K) { // from を含まない bit set
		if (cache[set][from] >= 0) return cache[set][from];
		
		long min = Long.MAX_VALUE;
		for (int i = 0; i < K; i++) {
			if ((set & (1 << i)) != 0) {
				int nset = set & ~(1 << i);
				long tmp = dists[from][i] + dist(nset, i, dists, K);
				if (min > tmp) min = tmp;
			}
		}
		
//		debug(from + " " + Integer.toBinaryString(set) + " " + min);
		return cache[set][from] = min;
	}
	
	
	private long[] bfs(Map<Integer, Set<Integer>> outs, int from, final int N) {
		long[] ret = new long[N];
		...
		while (!queue.isEmpty()) {
			...
			for (int next : outs.get(current)) {
				long ndist = ret[current] + 1;
				if (ret[next] > ndist) {
					queue.offer(next);
					ret[next] = ndist;
				}
