		...
		depth[0] = 0;
		{
			...
			while (!queue.isEmpty()) {
				...
				for (int current : queue) {
					depth[current] = d;
					next.addAll(outs.get(current));
				}
				d++;
				queue = next;
			}
		}
		Map<Integer, BitSet> depthMap = new HashMap<>();
		for (int i = 0; i < N; i++) {
			int d = depth[i];
			BitSet bs = depthMap.containsKey(d) ? depthMap.get(d) : new BitSet();
			bs.set(i);
			depthMap.put(d, bs);
		}
		
		BitSet[] child = new BitSet[N];
		Deque<DfsEntry> stack = new LinkedList<>();
		stack.push(new DfsEntry(0, 0));
		child[0] = new BitSet();
		child[0].set(0);
		while (!stack.isEmpty()) {
			DfsEntry current = stack.peek();
			if (current.next < outs.get(current.pos).size()) {
				int next = outs.get(current.pos).get(current.next);
				stack.push(new DfsEntry(next, 0));
				child[next] = new BitSet();
				child[next].set(next);
				current.next++;
			} else {
				stack.pop();
				BitSet currentChild = child[current.pos];
				for (int c : outs.get(current.pos)) {
					currentChild.or(child[c]);
//					currentChild.set(c);
				}
				child[current.pos] = currentChild;
			}
		}
		
		@SuppressWarnings("unchecked")
		Map<Integer, Set<Integer>>[] childDepthMap = new HashMap[N];
		for (int i = 0; i < N ; i++) childDepthMap[i] = new HashMap<>();
		while (!stack.isEmpty()) {
			DfsEntry current = stack.peek();
			if (current.next < outs.get(current.pos).size()) {
				int next = outs.get(current.pos).get(current.next);
				stack.push(new DfsEntry(next, 0));
				Set<Integer> dc = childDepthMap[next].containsKey(depth[next]) ? childDepthMap[next].get(depth[next]) : new HashSet<>();
				dc.add(next);
				childDepthMap[next].put(depth[next], dc);
				current.next++;
			} else {
				stack.pop();
//				for (int c )
...
//		BitSet EMPTY = new BitSet();
		for (int i = 0; i < Q; i++) {
			int U = sc.nextInt() - 1;
			int D = sc.nextInt();
//			BitSet and = ((BitSet) depthMap.getOrDefault(D, EMPTY).clone());
////			debug(and + " " + child[U]);
//			and.and(child[U]);
//			out.println(and.cardinality());
			out.println(childDepthMap[U].getOrDefault(D, Collections.emptySet()).size());
		}
	}
	
	public static class DfsEntry {
		public final int pos;
		public int next;
		public DfsEntry(int x, int y) {
			this.pos = x;
			this.next = y;
		}
		@Override
		public boolean equals(Object obj) {
			if (obj instanceof DfsEntry) {
				DfsEntry other = (DfsEntry) obj; 
				return this.pos == other.pos && this.next == other.next;
			}
			return false;
		}
		@Override public int hashCode() { return Long.hashCode(pos) * 31 + Long.hashCode(next); }
		@Override public String toString() { return "(" + pos + "," + next + ")"; }
	}
