package oiwa.atcoder.beginner202.questionE3;
...
public class Main {
	...
	public void run(PrintWriter out) {
		...
		Map<Integer, List<InOut>> depthMap = new HashMap<>();
		InOut[] timing = new InOut[N];
		Deque<DfsEntry> stack = new LinkedList<>();
		stack.push(new DfsEntry(0, 0, 0, 0));
		int time = 0;
		while (!stack.isEmpty()) {
			time++;
			...
			if (current.next < outs.get(current.pos).size()) {
				...
				stack.push(new DfsEntry(next, 0, current.depth+1, time));
				current.next++;
			} else {
				stack.pop();
				List<InOut> list = depthMap.containsKey(current.depth) ? depthMap.get(current.depth) : new ArrayList<>();
				list.add(timing[current.pos] = new InOut(current.in, time));
				depthMap.put(current.depth, list);
			}
		}
		for (List<InOut> list : depthMap.values()) {
			list.sort((e1, e2) -> Long.compare(e1.in, e2.in));
//			for (InOut io : list) System.out.print(" " + io);
//			System.out.println();
		}
		
		List<InOut> EMPTY = Collections.emptyList();
		final int Q = sc.nextInt();
//		BitSet EMPTY = new BitSet();
		for (int i = 0; i < Q; i++) {
			int U = sc.nextInt() - 1;
			int D = sc.nextInt();
			List<InOut> list = depthMap.getOrDefault(D, EMPTY);
			if (list.size() > 0) {
				long res = AtMath.binarySearchUpperBound(0, list.size()-1, timing[U].out, e -> list.get((int) e).out)
						- AtMath.binarySearchLowerBound(0, list.size()-1, timing[U].in , e -> list.get((int) e).in);
				if (res < 0) res = 0;
				out.println(res);
			} else {
				out.println(0);
	...
	public static class DfsEntry {
		...
		public final int in;
		public int next;
		public DfsEntry(int pos, int next, int depth, int in) {
			this.pos = pos;
			this.next = next;
			this.depth = depth;
			this.in = in;
	...
	public static class InOut {
		...
		@Override public int hashCode() { return Long.hashCode(in) * 31 + Long.hashCode(out); }
