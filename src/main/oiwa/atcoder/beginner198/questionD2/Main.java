...
public class Main {
	...
	public void run(PrintWriter out) {
		...
		List<Character> charList = chars.stream().collect(Collectors.toList());
		
		Map<Character, Integer> c2i = new HashMap<>();
		Set<Integer> used = new HashSet<>();
		
		long[] res = recurcive(charList, 0, used, c2i, S1, S2, S3);
		if (res == null) {
	...
	private long[] recurcive(List<Character> vars, int index, Set<Integer> used, Map<Character, Integer> c2i, String s１, String s2, String s3) {
		if (index == vars.size()) { // 全部決定されている場合
			long n1 = str2long(s１, c2i);
			long n2 = str2long(s2, c2i);
			long n3 = str2long(s3, c2i);
		...
		for (int i = 0; i < 10; i++) {
			if (!used.contains(i)) {
				used.add(i);
				c2i.put(vars.get(index), i);
				long[] res = recurcive(vars, index+1, used, c2i, s１, s2, s3);
				if (res != null) return res;
				c2i.remove(vars.get(index));
				used.remove(i);
			}
		}
		return null;
	...
	private long str2long(String str, Map<Character, Integer> map) {
		if (map.get(str.charAt(0)) == 0) return -1; // 先頭0は許容されない
		long ret = 0;
