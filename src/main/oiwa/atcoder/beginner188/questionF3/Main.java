package oiwa.atcoder.beginner188.questionF3;
...
public class Main {
	...
	public void run(PrintWriter out) {
		...
		cache = new HashMap<>();
		cache.put(0L, X);
		cache.put(1L, Math.abs(X-1));
		out.println(recurcive(X, Y));
		
	}
	Map<Long, Long> cache;
	private long recurcive(long x, long y) {
		if (cache.containsKey(y)) return cache.get(y);
		
		long ret;
		if (y % 2 == 0) {
			ret = Math.min(Math.abs(x - y), recurcive(x, y / 2) + 1);
		} else {
			ret = AtMath.min(Math.abs(x - y),
					recurcive(x, (y+1) / 2) + 2,
					recurcive(x, (y-1) / 2) + 2);
		}
		cache.put(y, ret);
