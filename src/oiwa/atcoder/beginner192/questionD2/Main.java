...
//		out.println(sc.next());
		...
		if (X.length() == 1) {
			out.println((X.charAt(0) - '0' <= M) ? 1 : 0);
			return;
		}
		...
		long max = Math.round(Math.ceil(Math.pow(M / (double) (X.charAt(0) - '0'), 1.0d / (X.length() - 1)))) + 1;
		
		while (min < max - 1) {
			long mid = (min + max) / 2;
			long v = eval(mid, X);
			debug(min + " " + mid + " " + max + " : " + v + " v<=M?" + (v<=M));
			if (v <= M) {
				min = mid;
			} else {
				max = mid;
	...
	private long eval(long d, String X) {
		final int xlen = X.length();
		BigInteger val = BigInteger.ZERO;
		for (int i = 0; i < xlen; i++) {
			val = val.multiply(BigInteger.valueOf(d)).add(BigInteger.valueOf(X.charAt(i) - '0'));
//			debug(val.longValue());
		}
		long v;
