package oiwa.atcoder.beginner192.questionD3;
...
//		out.println(sc.next());
		...
		char[] cX = sc.next().toCharArray();
		long M = sc.nextLong();
		
		int[] X = new int[cX.length];
		
		if (cX.length == 1) { // 基数に依らない
			int d = cX[0] - '0';
			if (d <= M) out.println(1);
			else out.println(0);
			return;
		}
		...
		long dMaxLim = Math.round(Math.pow(M, 1.0d / (X.length - 1))) + 10;
		
		long dMax = AtMath.binarySearchUpperBound(d, dMaxLim, M, v -> value(X, v)) - 1;
		if (dMax < d) {
			out.println(0);
			return;
		}
	...
	private long value(int[] X, long d) {
		long ret = 0;
		for (int i : X) {
			if (Long.MAX_VALUE / d < ret) return Long.MAX_VALUE;
			ret *= d;
			if (Long.MAX_VALUE - i < ret) return Long.MAX_VALUE;
			ret += i;
		}
		return ret;
	}
